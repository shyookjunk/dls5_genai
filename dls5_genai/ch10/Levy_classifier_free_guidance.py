...
class Diffuser():
  ...
  def sample(self,model,x_shape=(20,1,28,28),labels=None,gamma=0.0):
    batch_size=x_shape[0]
    #x=torch.randn(x_shape,device=self.device)
    x=Levy_dist_multidim(x_shape,c=1.0,alpha=1.0)
...
def show_images(images,rows=2,cols=10,labels=None):
...
def Levy_dist_multidim(size,c=1.0,alpha=1.5):
  u=np.zeros(size)
  dim_ary=len(size)
  num_element=1
  for i in size:
    num_element*=i
  u=u.reshape((1,num_element))
  for i in range(num_element):
    u[0,i]=Levy_dist(c,alpha)
...
# plt.show()
gamma=3.0
images,labels=diffuser.sample(model,gamma=gamma)
